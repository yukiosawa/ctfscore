...
<script>
    $(function(){
        $('#puzzle-table').tablesorter().floatThead({zIndex: 100});
        ...
        $('#ctfscore-puzzle-modal').on('hidden.bs.modal', function (e) {
            ...
            $('.ctfscore-answer-container').show();
        ...
        $('#ctfscore-puzzle-modal').on('show.bs.modal', function (e) {
            ...
            $.get('<?php echo Uri::base(false); ?>score/puzzle_view/' + $(e.relatedTarget).data('id'), function (response) {
            ...
            $('.ctfscore-answer').data('id', $(e.relatedTarget).data('id'));
            if ($(e.relatedTarget).data('answered') == 1) {
                $('.ctfscore-answer-container').hide();
                $('.ctfscore-hint-request').hide();
                $('.ctfscore-hint-container').hide();
            }
        });
        ...
        $('#ctfscore-puzzle-solvers-modal').on('show.bs.modal', function (e) {
            var self = $(this);
            $.get('<?php echo Uri::base(false); ?>score/puzzle_solvers/' + $(e.relatedTarget).data('id'), function (response) {
        ...
        $('.ctfscore-hint-request').click(function (e) {
            e.preventDefault();
            var self = $(this);
            $.get('<?php echo Uri::base(false); ?>hint/token', function (response) {
        ...
        $('.ctfscore-hint').submit(function (e) {
            e.preventDefault();
            var self = $(this);
            var action = '<?php echo Uri::base(false); ?>hint/create/' + self.data('id');
...
<?php if ($is_admin): ?>
        ...
        $('#ctfscore-hint-modal').on('show.bs.modal', function (e) {
            var self = $(this);
            $.get('<?php echo Uri::base(false); ?>hint/view/' + $(e.relatedTarget).data('id'), function (response) {
...
<?php endif; ?>
        ...
        $('.ctfscore-answer').submit(function (e) {
            var self = $(this);
            $('input[name="puzzle_id"]', '.ctfscore-answer').val(self.data('id'));
        });
...
<div class="row">
  <div class="col-md-12">
    ...
    foreach ($puzzles as $puzzle)
    {
        ...
        // タイトル
        $answered = ($puzzle['answered']) ? 1 : 0;
        echo "<td data-target='#ctfscore-puzzle-modal' data-toggle='modal' data-id='" . $puzzle['puzzle_id'] . "' data-answered='" . $answered . "' class='anchor'>" . $puzzle['title'] . "</td>";
        // ポイント
        ...
        // レビュー平均スコア
        echo "<td><a href='".Uri::base(false)."review/list/".$puzzle_id."'><div class='review' data-number='".\Model_Config::get_value('max_review_score')."' data-score='".$puzzle['avg_score']."'><span style='display:none'>".$puzzle['avg_score']."</span></div></a></td>";
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <div class="text-left ctfscore-answer-container">
                    <form action="<?php echo Uri::base(false); ?>score/submit" method="post" class="form-inline ctfscore-answer">
                        <div class="input-group">
                            <input name="puzzle_id" type="hidden">
                            <input name="answer" type="text" class="form-control" placeholder="flag を入力">
                                <span class="input-group-btn">
                                    <input type="submit" class="btn btn-primary" value="回答">
                                </span>
                        </div>
                    </form>
                </div>
